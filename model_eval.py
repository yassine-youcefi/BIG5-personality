...
class ModelEvaluator():
        ...
        for trait in traits:
            if model == 'RandomForestRegressor':
                ...
                rf_GSCV = GridSearchCV(
                    estimator=rf, param_grid=random_grid, cv=5)
    ...
    def compare_scores(self, models, regression=False):
        print('Model performance for trait ' +
              self.trait + ' prediction:' + '\n')
        ...
        for model_name in models:
            ...
            if regression:
                ...
                y_true = self.y_test
                mse = -np.mean(cross_validate(model, self.X_test, self.y_test,
                                              scoring='neg_mean_squared_error', cv=10)['test_score'])
                print('MSE: ' + str(mse) + '\n')
            else:
                accuracy_score = np.mean(cross_validate(
                    model, self.X_test, self.y_test, cv=10)['test_score'])
                ...
                f_score = np.mean(cross_validate(
                    model, self.X_test, self.y_test, scoring='f1', cv=10)['test_score'])
        ...
        if regression:
            ...
            best_accuracy_model = models[accuracy_scores.index(
                best_accuracy_score)]
            print(
