...
class Predictor():
    ...
    def predict_fb_statuses(self):
        statuses = list(self.fb_statuses.find({'friends_dict': {'$exists': False}, 'my_personality': {
                        '$exists': False}}, {'statuses': 1, '_id': 1, 'name': 1}))
    ...
    def insert_avgs_into_db(self):
        ...
        for status_idx, status in enumerate(avg_values):
            ...
            print('Inserting ' + name +
                  "'s average personality scores to database...")
            avg_personality = {}
    ...
    def add_profile_pic(self):
        ...
        for entry in entries:
            ...
            try:
                profile_pic_url = soup.select(
                    'a[class*=profilePicThumb] img')[0]['src']
            except:
    ...
    def add_percentiles(self):
        ...
        avg_scores_labels = ['avg_pred_sOPN', 'avg_pred_sCON',
                             'avg_pred_sEXT', 'avg_pred_sAGR', 'avg_pred_sNEU']
        scores_labels = ['pred_sOPN', 'pred_sCON',
                         'pred_sEXT', 'pred_sAGR', 'pred_sNEU']
        big5_labels = ['O_score', 'C_score', 'E_score', 'A_score', 'N_score']
        percs_labels = ['pred_perc_sOPN', 'pred_perc_sCON',
                        'pred_perc_sEXT', 'pred_perc_sAGR', 'pred_perc_sNEU']
            ...
            try:
                ...
                for idx, trait_label in enumerate(avg_scores_labels):
                    ...
                    perc = stats.percentileofscore(
                        self.df[scores_labels[idx]], score)
    ...
    def submit_personality_test(self, answers):
        ...
        self.create_plot(
            list(scores['percentiles'].values()), 'My_Personality')
